...
- `현재 HEAD`: `a27dccb5e6a086e7566fe23a3c4f5ae1f04c8559`
...
- 이전 기준: 계산기/입력 경계 정리
- 현재 HEAD: `ReadingGoalMetricsUseCase`로 계산 위임 (동일 계산 규칙)
- 판정: `구조`
- 권고안: 현재 구조 유지
- 승인 상태: `CONFIRMED (2026-02-17, 현재 HEAD 기준)`
...
- 현재 HEAD: 동일
- 판정: `개선`
- 권고안: 현재 HEAD 확정 (무의미한 입력 방지 + 날짜 경계 정책 일관)
- 승인 상태: `CONFIRMED (2026-02-17, 현재 HEAD 기준)`
...
3. `F-12` 미완독 종료: 완료상태 + 완료일/시작일 보정 + 알림 해제 확정
4. `F-13` 알림 OFF 상태 시간 변경: 시간 저장만 수행, 재등록 미실행 확정
5. `F-14` 알림 계산/재등록: 강화된 하한/페이지 계산 + 재등록 전 권한 재검증 확정
6. `F-15` 삭제 후 알림 정책: 남은 읽는 책 재설정/없으면 전체 해제 확정
7. `F-17` 조회 경계: 1회 마이그레이션 write-back 허용 확정

최종 제품 명세 적용 규칙:

1. 기본 계약은 `docs/product/past/main-feature-baseline.md`를 따른다.
2. 위 확정 항목(`F-08`, `F-09`, `F-12`, `F-13`, `F-14`, `F-15`, `F-17`)은 main 계약보다 우선한다.
