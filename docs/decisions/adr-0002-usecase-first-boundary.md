...
- 완료: `MainHomeViewModel`은 인프라(`NotificationManaging`) 직접 의존 없이 `HomeNotificationUsing` 경계를 통해 알림 트리거를 실행한다.
...
- 완료: `NotificationManaging`/`NotificationSettingsStoring` 선언은 Presentation(`NotificationSettingViewModel`)에서 분리되어 `Domain/Service`로 이동했고, `UserDefaultsNotificationSettingsStore` 구현은 `Data/UserDefaults`로 이동해 Presentation의 인프라 계약 소유를 제거했다.
